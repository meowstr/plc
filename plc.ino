...
static void render_io() {
  ...
  lcd.setCursor(0, 1);
  for (int i = 0; i < 8; i++) lcd.print((lastIoA >> i & 1) ? '1' : '.');
  for (int i = 0; i < 8; i++) lcd.print((lastIoB >> i & 1) ? '1' : '.');
}
...
static int read_button() {
  ...
  for (int i = 0; i < 6; i++) {
    if (abs(value - BUTTON_VALUE_LIST[i]) < abs(value - BUTTON_VALUE_LIST[best])) {
      best = i;
    }
  }
  return best;
...
void loop() {
  ...
  if (last_button == BUTTON_NONE) {
    ...
    if (button == BUTTON_LEFT) cx--;
    if (button == BUTTON_RIGHT) cx++;
    if (button == BUTTON_UP) cy--;
    if (button == BUTTON_DOWN) cy++;
    cx = constrain(cx, 0, 15);
    cy = constrain(cy, 0, 1);
    if (button == BUTTON_SELECT) {
      mcp.digitalWrite(1, LOW);
      static int toggle = 0;
      if (toggle) lcd.noBlink();
      if (!toggle) lcd.blink();
      if (toggle) mcp.digitalWrite(0, HIGH);
      if (!toggle) mcp.digitalWrite(0, LOW);
      toggle = (toggle + 1) % 2;
    } else {
      mcp.digitalWrite(1, HIGH);
    }
